...
def mixColumns(a, b, c, d):
    i=hex(galoisMult(a, 2) ^ galoisMult(b, 3) ^ galoisMult(c, 1) ^ galoisMult(d, 1))
    j=hex(galoisMult(a, 1) ^ galoisMult(b, 2) ^ galoisMult(c, 3) ^ galoisMult(d, 1))
    k=hex(galoisMult(a, 1) ^ galoisMult(b, 1) ^ galoisMult(c, 2) ^ galoisMult(d, 3))
    l=hex(galoisMult(a, 3) ^ galoisMult(b, 1) ^ galoisMult(c, 1) ^ galoisMult(d, 2))
    print(i,j,k,l)
    return [i,j,k,l]
...
for i in range(4):
    ...
    temp2 = []
    for j in range(4):
        temp2.append(((plainText[total])))
        temp.append(((plainText[total])))
        temp1.append(((key[total])))
        total+=1
